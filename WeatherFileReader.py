    ...
    def read_file(self, weather_file_names, path):
        ...
        for weather_file_name in weather_file_names:
            with open(path + weather_file_name, "r") as weather_file:
                ...
                weather_records_per_file = self.covert_weather_records_in_required_format(
                    weather_headings, weather_records_per_file
                )
                weather_records.extend(weather_records_per_file)

        return weather_records
    
    def covert_weather_records_in_required_format(self, weather_headings, weather_records_per_file):
        
        return [
            {
                weather_headings[index]: heading
                for index, heading in enumerate(record.strip().split(sep=","))
            }
